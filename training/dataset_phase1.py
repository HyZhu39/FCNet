...
class MakeupDataset(Dataset):
    def __init__(self, config=None):
        ...
        '''
        self.makeup_names = os.listdir('/home/ubuntuu/hdd/Dataset/CelebA-HQ-img_train/') #os.listdir('/home/ubuntuu/hdd/Dataset/CelebA-HQ-img/')
        '''
        scaner = scandir(dir_path=self.gt_path, suffix=IMG_EXTENSIONS, recursive=True, full_path=True) # /home/ubuntuu/hdd/Dataset/ffhq/images1024x1024/69000/69997.png
        paths = []
        paths += sorted(scaner)
        self.makeup_names = paths
        
        self.flip_rate = 0.5
        self.aug_rate = 0.75 #0.75
        ...
        self.transform_jitter = transforms.Compose([
            transforms.Resize(config.DATA.IMG_SIZE),
            transforms.ColorJitter(0.1, 0.1, 0.1, 0.1), #transforms.ColorJitter(0.2, 0.3, 0.5, 0.5), transforms.ColorJitter(0.3, 0.3, 0.5, 0.5), #transforms.ColorJitter(0.1, 0.1, 0.1, 0.1),
            transforms.ToTensor(),
            transforms.Normalize([0.5,0.5,0.5],[0.5,0.5,0.5])]) # 0,1
    ...
    def load_from_file_gt(self, img_name): # 'non-makeup/19946'
        ...
        #image = Image.open(os.path.join('/home/ubuntuu/hdd/Dataset/CelebA-HQ-img/', f'{base_name}.jpg')).convert('RGB')
        #mask = self.preprocessor.load_mask(os.path.join('/home/ubuntuu/hdd/Dataset/Seg_CelebA-HQ-img/CelebA-HQ-img/', f'{base_name}.png'))
        ...
        return self.preprocessor.process_train(image, mask) # self.preprocessor.process(image, mask, lms)
    ...
    def __getitem__(self, index):
        ...
        idx_refs = 5
        name_r = self.makeup_names[idx_r] # /home/ubuntuu/hdd/Dataset/ffhq/images1024x1024/04000/04507.png

        reference = self.load_from_file_gt(name_r) #self.load_from_file(name_r)
        
        mask = reference[1] # mask torch.Size([5, 256, 256])
        mask = remake_masks(torch.unsqueeze(mask, 0))[0]
        
        #reference_gt_ = Image.open(os.path.join(self.gt_path, name_r.split('/')[-1].replace('.png', '.jpg'))).convert('RGB')     # jpg
        ...
        reference_jitters = []
        reference_jitter_greys = []
        reference_jitter_masks = []
        reference_jitters_asgt = []

        for i in range(idx_refs): # 在这里flip
        
            #jitters, jitters_greys = self.convert_part_color(os.path.join(self.gt_path, name_r.split('/')[-1].replace('.png', '.jpg')), mask=mask, rate=self.rate)
            jitters, jitters_greys = self.convert_part_color(name_r, mask=mask, rate=self.rate)
        
            reference_jitters_asgt.append(jitters)
            reference_jitter_greys.append(jitters_greys)
            '''
            if np.random.rand() < self.flip_rate:
                reference_jitters.append(jitters)
                reference_jitter_masks.append(mask)
            else:
                reference_jitters.append(torch.flip(jitters, dims=[-1]))
                reference_jitter_masks.append(torch.flip(mask, dims=[-1]))
            ''' 
            #'''
            if np.random.rand() > self.flip_rate:
                jitters_ = jitters
                ...
                jitters_ = torch.flip(jitters, dims=[-1])
                mask_ = torch.flip(mask, dims=[-1])
            
            if (np.random.rand() > self.aug_rate): #or (i == idx_refs-1): # background不要加仿射变换
                reference_jitters.append(jitters_)
                reference_jitter_masks.append(mask_)
            else:
                angle = np.random.randint(-30, 30, size=1)[0] # -30,30
                ratio = np.around(random.uniform(1.3, 1.65), decimals=2) # 1.3 - 1.65 保留两位小数 
                jitters_, mask_ = affine_augmention(jitters_, mask_, angle, ratio)
                reference_jitters.append(jitters_)
                reference_jitter_masks.append(mask_)
            #'''
        
        reference_jitters = torch.stack(reference_jitters, dim=0)
        reference_jitter_greys = torch.stack(reference_jitter_greys, dim=0)
        reference_jitter_masks = torch.stack(reference_jitter_masks, dim=0)
        reference_jitters_asgt = torch.stack(reference_jitters_asgt, dim=0)
        
        # 这里也flip,首先得保证这里是正的
        reference_jitter_gt = self.merge_part_color(reference_jitters_asgt, mask=mask)
        reference_jitter_gt_grey = self.transform_grey(self.tensor_2_PIL(reference_jitter_gt))
        reference_jitter_gt_mask = mask
        
        reference_jitter_gt_asgt = reference_jitter_gt.clone()
        reference_jitter_gt_asgt_for_fakeB = reference_jitter_gt_grey.clone()
        #reference_jitter_gt_mask_for_fakeB = reference_jitter_gt_mask.clone()
        
        if np.random.rand() < self.flip_rate:
            reference_jitter_gt = torch.flip(reference_jitter_gt, dims=[-1])
            reference_jitter_gt_mask = torch.flip(reference_jitter_gt_mask, dims=[-1])
            reference_jitter_gt_asgt_for_fakeB = torch.flip(reference_jitter_gt_asgt_for_fakeB, dims=[-1])
        
        # 在此基础上，jitter和jitter_gt都得随机加flip增广（mask也得跟着转），（还得想想咋把jitter后结果对应的灰度图得到），input到里面的也应该是jitter后的灰度图
        # flip这还有点问题，目的是让他在给的ref和input相反时（ref被flip），可以用flip后的output直接和未flip的gt算loss才对
        # 无论如何，input的灰度图一定是正的，但是其ref及其mask可能是反的！
        
        return reference_gt, reference_grey, mask, reference_jitters, reference_jitter_greys, reference_jitter_masks, reference_jitters_asgt, reference_jitter_gt, reference_jitter_gt_grey, reference_jitter_gt_mask, reference_jitter_gt_asgt, reference_jitter_gt_asgt_for_fakeB

    def get_location(self, img, ratio=0.85): # PIL Image as input, ratio range is 0.85 ± 0.1
    ...
    def crop_with2location(self, img, location1, location2):
        ...
        img = torchvision.transforms.functional.crop(img, location1[1], location1[0], location1[3]-location1[1],
                                                 location1[2]-location1[0])  # top: int, left: int, height: int, width: int
    ...
    def convert_part_color(self, img_path, mask, rate):
        ...
        #mask = remake_masks(torch.unsqueeze(mask, 0))[0]    # torch.Size([5, 256, 256])
        ...
        if random_rate <= rate: # jitter
            ...
            return self.transform_jitter(img)
    '''
    ...
    def merge_part_color(self, jittered_imgs, mask):
        ...
        C_part, _, _ = mask.shape
        for ids in range(C_part):
            #print('jittered_imgs[ids]', jittered_imgs[ids].shape) # jittered_imgs[ids] torch.Size([3, 256, 256])
...
def affine_augmention(img, mask, angle, ratio):
    ...
    #Cm, Hm, Wm = mask.shape
    #aug_sizem = torch.Size([1, Cm, int(Hm * ratio), int(Wm * ratio)])
    gridm = F.affine_grid(theta.unsqueeze(0), aug_size, align_corners=True) # 为啥这里得用aug_size
