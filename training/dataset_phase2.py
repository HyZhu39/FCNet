...
class MakeupDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #jitter, jitter_grey = self.convert_part_color(os.path.join(self.gt_path, name_r.split('/')[-1].replace('.png', '.jpg')), mask=mask, rate=self.rate)
        jitter, jitter_grey = self.convert_part_color(name_r, mask=mask, rate=self.rate)
        
        if np.random.rand() > self.flip_rate:
            jitters_ = jitter
            mask_ = mask
        else:
            jitters_ = torch.flip(jitter, dims=[-1])
            mask_ = torch.flip(mask, dims=[-1])
            
        if (np.random.rand() > self.aug_rate): # background不要加仿射变换
            jitters__ = jitters_
            mask__ = mask_
        else:
            angle = np.random.randint(-30, 30, size=1)[0] # -30,30
            ratio = np.around(random.uniform(1.3, 1.65), decimals=2) # 1.3 - 1.65 保留两位小数 
            jitters__, mask__ = affine_augmention(jitters_, mask_, angle, ratio)

            
        return reference_gt, reference_grey, mask, jitters__, mask__, jitter
