...
# get landmarks with pretrained dlib
...
class PreProcess:
    ...
    def __init__(self, config, need_parser=True, device='cpu'):
        ...
        # NEW
        self.hair_class  = config.PREPROCESS.HAIR_CLASS
    ...
    def mask_process(self, mask: torch.Tensor):
        ...
        mask_face = (mask == self.face_class[0]).float() + (mask == self.face_class[1]).float() + (mask == self.face_class[2]).float() + (mask == self.face_class[3]).float() + (mask == self.face_class[4]).float()
        ...
        mask_hair = (mask == self.hair_class[0]).float() 

        mask_list = [mask_lip, mask_face, mask_eye_left, mask_eye_right, mask_hair]
    ...
    def preprocess_test(self, image: Image):
        ...
        if image.size[-1] != self.img_size:
            image = image.resize((self.img_size, self.img_size), Image.ANTIALIAS)
        return self.transform(image), self.mask_process(mask)
    ...
    def process_train(self, image: Image, mask: torch.Tensor):
        image = self.transform(image)
        mask = self.mask_process(mask)
        return [image, mask]
