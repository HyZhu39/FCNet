...
def generate_pgt(image_s, image_r, mask_s, mask_r, lms_s, lms_r, margins, blend_alphas, img_size=None):
        """
        input_data: (3, h, w)
        mask: (c, h, w), lip, skin, left eye, right eye !, hair!
        # NEW
        add hair in mask
        """
        if img_size is None:
        ...
        hair_match = masked_his_match(image_s, image_r, mask_s[4:5], mask_r[4:5])
        pgt = (1 - mask_s[4:5]) * pgt + mask_s[4:5] * hair_match
        mask_s_eye = mask_s[2:4].sum(dim=0, keepdim=True)
        mask_r_eye = mask_r[2:4].sum(dim=0, keepdim=True)
...
class ComposePGT(nn.Module):
    def __init__(self, margins, skin_alpha, eye_alpha, lip_alpha, hair_alpha):
...
class AnnealingComposePGT(nn.Module):
    def __init__(self, margins,
            ...
            hair_alpha_milestones, hair_alpha_values
        ):
        ...
        # NEW
        self.hair_alpha_fn = LinearAnnealingFn(hair_alpha_milestones, hair_alpha_values)
    ...
    def step(self):
        ...
        # NEW
        self.blend_alphas['hair'] = self.hair_alpha_fn(self.t)
...
class MakeupLoss(nn.Module):
    ...
    def forward(self, x, target, mask=None):
        if mask is None:
            ...
            mask_1 = mask.clone()
            mask_2 = mask.clone()
            return F.l1_loss(x * mask_1, target * mask_2)
